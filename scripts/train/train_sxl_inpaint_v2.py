"""
ControlNet for Mask-Conditioned Inpainting with Stable Diffusion XL (V2)

Training approach:
1. Load SDXL base model (frozen)
2. Initialize ControlNet from scratch with mask-only input
...
- ControlNet: Trained from scratch, conditioned on mask only (1 channel, padded to 4)
- UNet: Receives masked image (encoded to latents) + empty prompt
- Input: Binary mask (1 = hole to inpaint, 0 = valid region)
- Output: Complete inpainted image

Key difference from V1:
- ControlNet receives only mask (not mask + masked image)
- UNet receives masked image latents (from conds) + empty prompt embeddings
- This separates mask conditioning (ControlNet) from image conditioning (UNet)
"""
...
class InpaintPairDataset(Dataset):
    ...
    - image_cond.png: Masked/incomplete image (used for UNet input)
    - mask_cond.png: Binary mask (used for ControlNet input)
    ...
    UNet receives the masked image and empty prompt.
    """
    ...
    with torch.no_grad():
        for idx, sample in enumerate(val_samples):
            try:
                ...
                masked_latents = pipe.vae.encode(conds * 2 - 1).latent_dist.sample() * 0.18215
                
                # ControlNet conditioning: mask only (padded to 4 channels)
                controlnet_cond = torch.cat([masks, torch.zeros_like(conds)], dim=1)  # [1, 4, H, W]
                ...
                noisy_masked_latents = pipe.scheduler.add_noise(masked_latents, torch.randn_like(masked_latents), timesteps)
    ...
    with torch.no_grad():
        for idx, sample in enumerate(val_samples):
            try:
                ...
                # Prepare control image: mask only (padded to 4 channels for ControlNet)
                # Convert to tensor format that ControlNet expects [1, 4, H, W]
                mask_4ch = torch.cat([mask, torch.zeros_like(cond_image)], dim=1)  # [1, 4, H, W]
                
                # Convert masked image to PIL for passing as image parameter (if needed)
                cond_image_np = cond_image.squeeze(0).permute(1, 2, 0).cpu().numpy()
                cond_image_pil = Image.fromarray((np.clip(cond_image_np, 0, 1) * 255).astype(np.uint8), mode='RGB')
                
                # Encode masked image to latents for UNet initialization
                cond_image_clamped = cond_image.clamp(0, 1)
                masked_latents = pipe.vae.encode(cond_image_clamped * 2 - 1).latent_dist.sample() * 0.18215
                
                # Prepare initial latents from masked image with noise
                inference_pipe.scheduler.set_timesteps(num_inference_steps, device=device)
                timesteps = inference_pipe.scheduler.timesteps
                noise = torch.randn_like(masked_latents, generator=generator)
                initial_latents = inference_pipe.scheduler.add_noise(masked_latents, noise, timesteps[0:1])
                
                # Run inference using pipeline API
                # image parameter is the control image (mask as 4-channel tensor)
                # The pipeline will handle the masked image through latents parameter
                result = inference_pipe(
                    prompt="",
                    negative_prompt="",
                    image=mask_4ch,  # Control image: mask (4 channels: mask + 3 zeros)
                    num_inference_steps=num_inference_steps,
                    guidance_scale=guidance_scale,
                    generator=generator,
                    latents=initial_latents,  # Initial latents from masked image (for UNet)
                    height=mask.shape[2],
                    width=mask.shape[3]
                ).images[0]
                ...
                # Conditioning Image (masked image)
                cond_image_np = cond_image.squeeze().permute(1, 2, 0).cpu().numpy()
                axes[1].imshow(np.clip(cond_image_np, 0, 1))
    ...
    with torch.no_grad():
        for idx in tqdm(range(len(test_dataset)), desc="Evaluation"):
            try:
                ...
                # ControlNet conditioning: mask only (padded to 4 channels)
                controlnet_cond = torch.cat([masks, torch.zeros_like(conds)], dim=1)
                ...
                down_block_res_samples, mid_block_res_sample = controlnet(
                    noisy_masked_latents, timesteps,
                    encoder_hidden_states=prompt_embeds,
                    controlnet_cond=controlnet_cond,
                    added_cond_kwargs=added_cond_kwargs,
                    return_dict=False
                ...
                if idx < 20:
                    # Prepare control image: mask only (padded to 4 channels for ControlNet)
                    mask_4ch_eval = torch.cat([masks, torch.zeros_like(conds)], dim=1)  # [1, 4, H, W]
                    
                    # Encode masked image to latents for UNet initialization
                    conds_clamped = conds.clamp(0, 1)
                    masked_latents_eval = pipe.vae.encode(conds_clamped * 2 - 1).latent_dist.sample() * 0.18215
                    
                    # Prepare initial latents from masked image with noise
                    inference_pipe.scheduler.set_timesteps(num_inference_steps, device=device)
                    timesteps_eval = inference_pipe.scheduler.timesteps
                    noise_eval = torch.randn_like(masked_latents_eval, generator=generator)
                    initial_latents_eval = inference_pipe.scheduler.add_noise(masked_latents_eval, noise_eval, timesteps_eval[0:1])
                    
                    # Run inference using pipeline API
                    result = inference_pipe(
                        prompt="",
                        negative_prompt="",
                        image=mask_4ch_eval,  # Control image: mask (4 channels: mask + 3 zeros)
                        num_inference_steps=num_inference_steps,
                        guidance_scale=guidance_scale,
                        generator=generator,
                        latents=initial_latents_eval,  # Initial latents from masked image (for UNet)
                    ...
                    cond_np = conds.squeeze().permute(1, 2, 0).cpu().numpy()
...
def main(cfg_path="configs/train_config.yaml"):
    ...
    print("🔧 Initializing ControlNet from scratch (mask-only conditioning)...")
    controlnet = ControlNetModel.from_unet(
        pipe.unet,
        conditioning_channels=4  # 1 mask channel + 3 zeros = 4 total (for compatibility)
    ).to(device)
    ...
    for epoch in range(cfg["train"]["epochs"]):
        ...
        for step, batch in enumerate(tqdm(loader, desc=f"Epoch {epoch}")):
            try:
                ...
                with torch.no_grad():
                    ...
                    # Encode target images to latents with fp32 VAE (for loss computation)
                    latents = pipe.vae.encode(images * 2 - 1).latent_dist.sample() * 0.18215
                    
                    # Encode masked image (conds) to latents (for UNet input)
                    masked_latents = pipe.vae.encode(conds * 2 - 1).latent_dist.sample() * 0.18215
                    
                    # ControlNet conditioning: mask only (1 channel)
                    # Pad to 4 channels to match ControlNet architecture (mask + 3 zeros)
                    controlnet_cond = torch.cat([masks, torch.zeros_like(conds)], dim=1)  # [B, 4, H, W]
                    
                    # Encode empty prompt for UNet
                    prompt_embeds, pooled_prompt_embeds = pipe.encode_prompt(
                        prompt="",
                        prompt_2="",
                        device=device,
                    ...
                    # prompt_embeds: [B, 77, cross_attention_dim]
                    # pooled_prompt_embeds: [B, projection_dim]
                    
                # Keep latents in fp32 for ControlNet training stability

                # Check for abnormal latent magnitudes
                if not torch.isfinite(latents).all() or not torch.isfinite(masked_latents).all():
                    print(f"⚠️ Non-finite latents detected, skipping.")
                    continue
                    
                if latents.abs().max() > 100 or masked_latents.abs().max() > 100:
                    print(f"⚠️ Latent explosion detected, skipping.")
                    continue
                ...
                # Add noise to masked latents (for UNet input)
                noisy_masked_latents = pipe.scheduler.add_noise(masked_latents, torch.randn_like(masked_latents), timesteps)

                # 3️⃣ Get ControlNet output (in fp32)
                # Prepare conditioning for SDXL with empty prompt
                added_cond_kwargs = {
                    "text_embeds": pooled_prompt_embeds,
                    "time_ids": torch.zeros(
                        (latents.shape[0], 6),
                ...
                down_block_res_samples, mid_block_res_sample = controlnet(
                    noisy_masked_latents,  # Use noisy masked latents for ControlNet
                    timesteps,
                    encoder_hidden_states=prompt_embeds,
                    controlnet_cond=controlnet_cond,
                    added_cond_kwargs=added_cond_kwargs,
                ...
                # UNet forward pass with masked image latents and empty prompt
                model_pred = pipe.unet(
                    noisy_masked_latents,  # Use noisy masked latents (from conds)
                    timesteps,
                    encoder_hidden_states=prompt_embeds,  # Empty prompt embeddings
                    added_cond_kwargs=added_cond_kwargs,
                    down_block_additional_residuals=down_block_res_samples,
                    mid_block_additional_residual=mid_block_res_sample,
                ).sample
        ...
        if validate_every > 0 and (epoch + 1) % validate_every == 0:
            ...
            # Randomly select DIFFERENT test samples each epoch
